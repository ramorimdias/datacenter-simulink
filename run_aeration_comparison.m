...
%   results/aeration_pumping_comparison.csv
    ...
    internal_liquid_flow_m3h = zeros(n,1);
    internal_mixture_flow_m3h = zeros(n,1);
    annual_pump_energy_kWh = zeros(n,1);
    annual_pump_cost = zeros(n,1);
    period_PUE = zeros(n,1);

    % The top-level builder expands visible Internal/External labels after
    % constructing the model, so use the resulting saved block paths here.
    internal_alpha_block = ...
        [model '/Aeration Model/Internal Loop Air Volume Fraction'];
    external_alpha_block = ...
        [model '/Aeration Model/External Loop Air Volume Fraction'];
    ...
    for i = 1:n
        ...
        internal_liquid_flow_m3h(i) = time_average(liquid_flow_ts);
        internal_mixture_flow_m3h(i) = ...
            internal_liquid_flow_m3h(i) / (1-alpha);
    ...
    comparison = table( ...
        alpha_cases(:), ...
        100*alpha_cases(:), ...
        internal_liquid_flow_m3h, ...
        internal_mixture_flow_m3h, ...
        internal_pump_kW, ...
        external_pump_kW, ...
        total_pump_kW, ...
        delta_pump_power_kW, ...
        ...
        'AirVolumeFraction', ...
        'AirVolumePercent', ...
        'InternalUsefulLiquidFlow_m3h', ...
        'InternalPumpMixtureFlow_m3h', ...
