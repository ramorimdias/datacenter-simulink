...
% tuned during a normal-mode simulation from the dedicated upstream blocks.
    ...
    flow_expr = [ ...
        'u(1)*1000*3600/' ...
        '(max(u(2),1e-9)*internal_target_deltaT_K)'];
    ...
    pump_expr = [ ...
        ...
        '(((u(1)*1000/(max(u(2),1e-9)*internal_target_deltaT_K)/' ...
        'max(u(5),1e-6))/Vdot_internal_reference_m3s)^2)*' ...
        '(u(3)/rho_internal_kg_m3)*' ...
        '(max(u(4),1e-9)/mu_internal_reference_Pas)^mu_pressure_exponent)*' ...
        '(u(1)*1000/(max(u(2),1e-9)*internal_target_deltaT_K)/' ...
        'max(u(5),1e-6))/' ...
        '(internal_pump_efficiency*max(u(6),1e-6)*1000)'];
    add_block('simulink/User-Defined Functions/Fcn', ...
    ...
    chip_expr = [ ...
        ...
        'u(2)*1000/total_coldplate_paths*max(u(3),0)'];
    add_block('simulink/User-Defined Functions/Fcn', ...
