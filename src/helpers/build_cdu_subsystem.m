...
% tuned during a normal-mode simulation from the dedicated upstream blocks.
    ...
    add_block('simulink/User-Defined Functions/Fcn', [path '/Internal Pressure Drop Pa'], ...
        ...
        '((u(2)+1e-9)*internal_target_deltaT_K)/(u(5)+1e-6))/(3.14159265*u(8)^2))*' ...
        'u(8)/(u(4)+1e-9))^0.9))^2))*u(7)/u(8)+u(9))*u(3)*' ...
        '(4*(u(1)*1000/((u(2)+1e-9)*internal_target_deltaT_K)/(u(5)+1e-6))/(3.14159265*u(8)^2))^2/2+u(10)'], ...
