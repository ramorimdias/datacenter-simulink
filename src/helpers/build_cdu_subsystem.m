...
%
% The subsystem distinguishes useful liquid flow from total pump mixture flow.
% Aeration increases pump flow through the mixture-flow factor but does not
% create an artificial additional liquid-flow demand.

    add_in(path, 'HeatFromRacks_kW', 1, [25 45 55 65]);
    add_in(path, 'ExternalSupply_C', 2, [25 95 55 115]);
    add_in(path, 'InternalLiquidRhoCp_J_m3K', 3, [25 165 55 185]);
    add_in(path, 'InternalMixtureDensity_kg_m3', 4, [25 215 55 235]);
    add_in(path, 'InternalViscosity_Pa_s', 5, [25 265 55 285]);
    add_in(path, 'MixtureFlowFactor', 6, [25 315 55 335]);
    add_in(path, 'PumpEfficiencyRatio', 7, [25 365 55 385]);
    ...
    liquid_flow_expr = ['u(1)*1000*3600/(u(2)*' deltaT ')'];
    add_block('simulink/Signal Routing/Mux', [path '/Delivered Liquid Flow Inputs'], ...
        'Inputs', '2', 'Position', [270 45 290 105]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Delivered Liquid Flow m3_h'], ...
        'Expr', liquid_flow_expr, 'Position', [330 55 540 105]);

    add_block('simulink/Signal Routing/Mux', [path '/Pump Mixture Flow Inputs'], ...
        'Inputs', '2', 'Position', [300 125 320 185]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Pump Mixture Flow m3_h'], ...
        'Expr', 'u(1)*u(2)', 'Position', [350 135 540 175]);

    q_liq_expr = ['u(1)*1000/(u(2)*' deltaT ')'];
    q_mix_expr = ['(' q_liq_expr ')*u(5)'];
    velocity_expr = ['(' q_mix_expr ')/(pi*u(7)^2/4)'];
    re_expr = ['u(3)*(' velocity_expr ')*u(7)/u(4)'];
    r_safe = ['sqrt((' re_expr ')^2+1e-12)'];
    w_expr = ['0.5*(1+((' r_safe ')-4000)/sqrt(((' r_safe ')-4000)^2+1))'];
    f_expr = ['(1-(' w_expr '))*(64/(' r_safe '))+(' w_expr ...
        ')*0.25/(log10(u(8)/(3.7*u(7))+5.74/(' r_safe ')^0.9)^2)'];
    alpha_expr = '(1-1/u(5))';
    two_phase_multiplier_expr = ['(1+internal_two_phase_dp_coefficient*(' alpha_expr '))'];
    branch_ratio_expr = ['(' q_mix_expr ')/(u(11)*internal_reference_branch_flow_m3s)'];
    dp_expr = [two_phase_multiplier_expr '*(((' f_expr ')*u(6)/u(7)+u(9))*' ...
        'u(3)*((' velocity_expr ')^2)/2+u(10)*((' branch_ratio_expr ')^2))'];
    ...
    add_block('simulink/User-Defined Functions/Fcn', [path '/Internal Pressure Drop Pa'], ...
        'Expr', dp_expr, 'Position', [560 560 800 620]);

    add_block('simulink/Signal Routing/Mux', [path '/Pump Power Inputs'], ...
        'Inputs', '3', 'Position', [560 185 580 275]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Internal Pump Shaft Power kW'], ...
        'Expr', ['u(1)*u(2)/3600/(internal_pump_efficiency*u(3)*1000)'], ...
        'Position', [620 185 850 225]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Internal Pump Electrical Power kW'], ...
        'Expr', 'u/(internal_motor_efficiency*internal_vfd_efficiency)', ...
        'Position', [900 185 1135 225]);

    add_block('simulink/Signal Routing/Mux', [path '/Branch Flow Inputs'], ...
        'Inputs', '2', 'Position', [560 650 580 710]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Coldplate Branch Mixture Flow m3_h'], ...
        'Expr', 'u(1)/u(2)', 'Position', [620 660 870 700]);

    add_block('simulink/Math Operations/Sum', [path '/Heat to External kW'], ...
        'Inputs', '++', 'Position', [900 115 950 165]);
    ...
    diagnostics = {
        'Liquid Flow Display', [1040 55 1185 90], 'Delivered Liquid Flow m3_h';
        'Mixture Flow Display', [1040 115 1185 150], 'Pump Mixture Flow m3_h';
        'Pump Shaft Power Display', [1040 170 1185 205], 'Internal Pump Shaft Power kW';
        'Pump Electric Display', [1040 215 1185 250], 'Internal Pump Electrical Power kW';
        ...
        'Branch Flow Display', [890 650 1035 685], 'Coldplate Branch Mixture Flow m3_h'
    };
    ...
    add_line(path, 'InternalLiquidRhoCp_J_m3K/1', 'Hydraulic Inputs/2');
    add_line(path, 'InternalMixtureDensity_kg_m3/1', 'Hydraulic Inputs/3');
    add_line(path, 'InternalViscosity_Pa_s/1', 'Hydraulic Inputs/4');
    add_line(path, 'MixtureFlowFactor/1', 'Hydraulic Inputs/5');
    ...
    add_line(path, 'Pump Mixture Flow m3_h/1', 'Pump Power Inputs/2');
    add_line(path, 'PumpEfficiencyRatio/1', 'Pump Power Inputs/3');
    add_line(path, 'Pump Power Inputs/1', 'Internal Pump Shaft Power kW/1');
    add_line(path, 'Internal Pump Shaft Power kW/1', 'Internal Pump Electrical Power kW/1');

    add_line(path, 'HeatFromRacks_kW/1', 'Delivered Liquid Flow Inputs/1');
    add_line(path, 'InternalLiquidRhoCp_J_m3K/1', 'Delivered Liquid Flow Inputs/2');
    add_line(path, 'Delivered Liquid Flow Inputs/1', 'Delivered Liquid Flow m3_h/1');
    add_line(path, 'Delivered Liquid Flow m3_h/1', 'Pump Mixture Flow Inputs/1');
    add_line(path, 'MixtureFlowFactor/1', 'Pump Mixture Flow Inputs/2');
    add_line(path, 'Pump Mixture Flow Inputs/1', 'Pump Mixture Flow m3_h/1');

    add_line(path, 'HeatFromRacks_kW/1', 'Heat to External kW/1');
    add_line(path, 'Internal Pump Shaft Power kW/1', 'Heat to External kW/2');
    add_line(path, 'Pump Mixture Flow m3_h/1', 'Branch Flow Inputs/1');
    add_line(path, 'Total Coldplate Paths/1', 'Branch Flow Inputs/2');
    add_line(path, 'Branch Flow Inputs/1', 'Coldplate Branch Mixture Flow m3_h/1');
    ...
    connect_output(path, 'Delivered Liquid Flow m3_h', 'InternalFlow_m3h');
    connect_output(path, 'Internal Pump Electrical Power kW', 'InternalPump_kW');
    ...
    add_line(path, 'Delivered Liquid Flow m3_h/1', 'Liquid Flow Display/1');
    add_line(path, 'Pump Mixture Flow m3_h/1', 'Mixture Flow Display/1');
    add_line(path, 'Internal Pump Shaft Power kW/1', 'Pump Shaft Power Display/1');
    add_line(path, 'Internal Pump Electrical Power kW/1', 'Pump Electric Display/1');
    ...
    add_line(path, 'Coldplate Branch Mixture Flow m3_h/1', 'Branch Flow Display/1');
end
