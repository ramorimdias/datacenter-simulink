...
% tuned during a normal-mode simulation from the dedicated upstream blocks.
    ...
    add_block('simulink/Sources/Constant', [path '/Pipe Roughness m'], 'Value', ...
        'internal_pipe_roughness_m', 'Position', [175 500 275 525]);
    add_block('simulink/Sources/Constant', [path '/Fittings K'], 'Value', ...
        'internal_fittings_loss_coefficient', 'Position', [175 535 275 560]);
    add_block('simulink/Sources/Constant', [path '/Coldplate dP Pa'], 'Value', ...
        'coldplate_pressure_drop_Pa', 'Position', [175 570 275 595]);
    add_block('simulink/User-Defined Functions/Fcn', [path '/Internal Pressure Drop Pa'], ...
        'Expr', ['((0.3164/((u(3)*(4*(u(1)*1000/((u(2)+1e-9)*internal_target_deltaT_K)/' ...
        '(u(5)+1e-6))/(3.14159265*u(7)^2))*u(7)/(u(4)+1e-9))^0.25))*u(6)/u(7)+u(8))*u(3)*' ...
        '(4*(u(1)*1000/((u(2)+1e-9)*internal_target_deltaT_K)/(u(5)+1e-6))/(3.14159265*u(7)^2))^2/2+u(9)'], ...
    ...
    add_line(path, 'Coldplate dP Pa/1', 'Pressure Drop Inputs/10');
