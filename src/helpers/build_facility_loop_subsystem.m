...
% the operating point responds immediately to interactive parameter changes.
    ...
    flow_expr = [ ...
        'u(1)*1000*3600/' ...
        '(max(u(2),1e-9)*external_target_deltaT_K)'];
    ...
    pump_expr = [ ...
        ...
        '(((u(1)*1000/(max(u(2),1e-9)*external_target_deltaT_K)/' ...
        'max(u(5),1e-6))/Vdot_external_reference_m3s)^2)*' ...
        '(u(3)/rho_external_kg_m3)*' ...
        '(max(u(4),1e-9)/mu_external_reference_Pas)^mu_pressure_exponent)*' ...
        '(u(1)*1000/(max(u(2),1e-9)*external_target_deltaT_K)/' ...
        'max(u(5),1e-6))/' ...
        '(external_pump_efficiency*max(u(6),1e-6)*1000)'];
    add_block('simulink/User-Defined Functions/Fcn', ...
